...
context('PoolTogether mUSD Prize Pool', () => {
    ...
    before('reset block number', async () => {
        ...
        mAsset = Masset__factory.connect(mUSD.address, mUsdWhale1.signer);
        saving = ISavingsContractV2__factory.connect(mUSD.savings!, mUsdWhale1.signer);
        savingERC20 = ERC20__factory.connect(mUSD.savings!, mUsdWhale1.signer);
    ...
    it('Deploy mUSD Prize Pool', async () => {
        ...
        console.log(
            `${multipleWinnersConfig.sponsorshipName} (${multipleWinnersConfig.sponsorshipSymbol}) address: ${sponsorTokenAddress}`,
        );
    ...
    it('mUSD whale 1 deposits into pool', async () => {
        ...
        const savingCreditBalanceAfter = await savingERC20.balanceOf(mStableYieldSource.address);

        expect(savingCreditBalanceAfter).to.eq(
        ...
        const controlTokenAddress = await prizeStrategy.ticket();
        const [creditRateMantissa] = await prizePool.callStatic.creditPlanOf(controlTokenAddress);

        expect(
            await prizePool.callStatic.balanceOfCredit(mUsdWhale1Address, controlTokenAddress),
            'balanceOfCredit mUSD whale control after',
        ).to.eq(
            // deltaTime = currentTimestamp - userTimestamp = 1
            // (controlledToken balance * (deltaTime * creditRateMantissa)) / 1e18
            firstDepositAmount.mul(creditRateMantissa).div(simpleToExactAmount(1, 18)),
        );
    ...
    it('Move time ahead 6 days', async () => {
        ...
        const whale2MusdBalanceBefore = await mAsset.balanceOf(mUsdWhale2Address);
        ...
        const winnerAddress = awardEvent.args.winner;
        const winnerAmount = awardEvent.args.amount;

        if (winnerAddress === mUsdWhale1Address) {
            ...
            expect(await controlToken.balanceOf(mUsdWhale1Address), 'Whale 1 control balance after').to.eq(
                firstDepositAmount.add(winnerAmount),
            );

            expect(await controlToken.balanceOf(mUsdWhale2Address), 'Whale 2 control balance after').to.eq(secondDepositAmount);
        } else {
            expect(await mAsset.balanceOf(mUsdWhale2Address), "Winner mUSD balance doesn't change").to.eq(whale2MusdBalanceBefore);
            expect(await controlToken.balanceOf(mUsdWhale1Address), 'Whale 1 control balance after').to.eq(firstDepositAmount);
            expect(await controlToken.balanceOf(mUsdWhale2Address), 'Whale 2 control balance after').to.eq(
                secondDepositAmount.add(winnerAmount),
            );
        }
    ...
    it('Whale 1 withdraw deposit and some winnings', async () => {
        ...
        const withdrawAmount = whale1ControlBalanceBefore.sub(simpleToExactAmount(100));
